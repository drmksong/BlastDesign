...
class MkLine(MkObj):
    ...
    def calc_lm(self):
        ...
        assert (
            abs(L) > EPS or abs(M) > EPS
        ), f"MkLine::calc_lm() L { L } or M {M} should not be zero"
    ...
    def get_t(self, p: MkPoint) -> float:
        assert (
            abs(self.l) > EPS or abs(self.m) > EPS
        ), f"either l {self.l} or m {self.m} should not be zero"
    ...
    def is_inter(self, arc: "MkArc") -> bool:
        ...
        assert (
            type(arc.whang(pnt1)) != bool and type(arc.whang(pnt2)) != bool
    ...
    def get_inter(self, arc: "MkArc"):
        ...
        print(
            f"mkline::get_inter with arc pnt1 {pnt1} pnt2 {pnt2} t1 {t1} t2 {t2} ang1 {ang1} ang2 {ang2}"
        )
        ...
        if (
            (arc.sang < ang1 and ang1 < arc.eang)
            or (arc.sang + 360 < ang1 and ang1 < arc.eang + 360)
            or (arc.sang - 360 < ang1 and ang1 < arc.eang - 360)
        ):
            flag1 = True

        if (
            (arc.sang < ang2 and ang2 < arc.eang)
            or (arc.sang + 360 < ang2 and ang2 < arc.eang + 360)
            or (arc.sang - 360 < ang2 and ang2 < arc.eang - 360)
        ):
            flag2 = True
    ...
    def get_proj(self, c: "MkCircle"):
        ...
        if pnt1 is None and pnt2 is None:
            ...
            assert (
                abs(x2 - x1) + abs(y2 - y1) > EPS
            ), "MkLine::get_proj(circle) Strange line is almost a point, should be captured earlier"
            ...
            assert (
                b**2 - 4 * a * c > 0
            ), f"b**2 - 4*a*c is {b**2 - 4*a*c} which is less than zero, needs to be checked"
    ...
    def get_proj(self, arc: "MkArc"):
        ...
        if pnt1 is None and pnt2 is None:
            ...
            assert (
                b**2 - 4 * a * c > 0
            ), f"b**2 - 4*a*c is {b**2 - 4*a*c} which is less than zero"
            t1 = (-b + math.sqrt(b**2 - 4 * a * c)) / 2 / a
            t2 = (-b - math.sqrt(b**2 - 4 * a * c)) / 2 / a
    ...
    @dispatch
    def offset(
        self, dist: float, dir: mkDir
    ) -> "MkLine":  # dir : true : right, false : left
        x1 = self.p1.x
        x2 = self.p2.x
        y1 = self.p1.y
        y2 = self.p2.y
