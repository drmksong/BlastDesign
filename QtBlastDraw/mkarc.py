...
class MkArc(MkCircle):
    ...
    def is_inter(self, line: "MkLine") -> bool:
        ...
        assert (
            type(self.whang(pnt1)) != bool and type(self.whang(pnt2)) != bool
        ), f"MkArc::is_inter angle check failed pnt1 {self.whang(pnt1)} pnt2 {self.whang(pnt2)}"
    ...
    def get_inter(self, line: "MkLine"):
        ...
        assert (
            type(self.whang(pnt1)) != bool and type(self.whang(pnt2)) != bool
        ), "MkArc::is_inter angle check failed"
        ...
        if (
            (self.sang < ang1 and ang1 < self.eang)
            or (self.sang + 360 < ang1 and ang1 < self.eang + 360)
            or (self.sang - 360 < ang1 and ang1 < self.eang - 360)
        ):
            flag1 = True

        if (
            (self.sang < ang2 and ang2 < self.eang)
            or (self.sang + 360 < ang2 and ang2 < self.eang + 360)
            or (self.sang - 360 < ang2 and ang2 < self.eang - 360)
        ):
            flag2 = True
    ...
    def get_inter(self, c: "MkCircle"):
        ...
        assert (
            type(self.whang(p3)) != bool and type(self.whang(p4)) != bool
        ), "MkArc::is_inter angle check failed"
    ...
    def is_inter(self, a: "MkArc") -> bool:
        ...
        # TODO : This code is bug prone, must revise it to resilient code
        if (
            (a.sang < ang1 and ang1 < a.eang)
            or (a.sang + 360 < ang1 and ang1 < a.eang + 360)
            or (a.sang - 360 < ang1 and ang1 < a.eang - 360)
        ):
            flag1 = True

        if (
            (a.sang < ang2 and ang2 < a.eang)
            or (a.sang + 360 < ang2 and ang2 < a.eang + 360)
            or (a.sang - 360 < ang2 and ang2 < a.eang - 360)
        ):
            flag2 = True
        ...
        # TODO : This code is bug prone, must revise it to resilient code
        if (
            (self.sang < ang3 and ang3 < self.eang)
            or (self.sang + 360 < ang3 and ang3 < self.eang + 360)
            or (self.sang - 360 < ang3 and ang3 < self.eang - 360)
        ):
            flag3 = True

        if (
            (self.sang < ang4 and ang4 < self.eang)
            or (self.sang + 360 < ang4 and ang4 < self.eang + 360)
            or (self.sang - 360 < ang4 and ang4 < self.eang - 360)
        ):
            flag4 = True
    ...
    def get_inter(self, a: "MkArc"):
        ...
        # TODO : This code is bug prone, must revise it to resilient code
        if (
            (a.sang < ang3 and ang3 < a.eang)
            or (a.sang + 360 < ang3 and ang3 < a.eang + 360)
            or (a.sang - 360 < ang3 and ang3 < a.eang - 360)
        ):
            flag3 = True

        if (
            (a.sang < ang4 and ang4 < a.eang)
            or (a.sang + 360 < ang4 and ang4 < a.eang + 360)
            or (a.sang - 360 < ang4 and ang4 < a.eang - 360)
        ):
            flag4 = True
    ...
    def dist(self, p: MkPoint) -> float:
        ...
        assert (
            pnt1 is not None or pnt2 is not None
        ), "MkArc::dist wrt point, both points are None"
    ...
    def dist(self, l: "MkLine") -> float:
        ...
        assert (
            pnt1 is not None or pnt2 is not None
        ), "MkArc::dist wrt line, both points are None"
    ...
    def trim(self, l: "MkLine", dir: mkDir):
        ...
        flag = (
            pnt1 != None and pnt2 != None
        )  # two points then true, one or less then false

        assert (
            pnt1 != None or pnt2 != None
        ), f"MkArc::trim with line, it is strange both pnt1 {pnt1} and pnt2 {pnt2}"
        ...
        if (dir & mkDir.mkIN) == mkDir.mkIN:
            assert (
                flag
            ), f"MkArc::trim with line, it is strange dir is mkIN but intersections should be two pnts"
        ...
        if flag:  # two intersection points
            if (mkDir.mkOUT | mkDir.mkEND | mkDir.mkSTART) == dir & (
                mkDir.mkOUT | mkDir.mkEND | mkDir.mkSTART
            ):
                line1.p1 = pnt1
                line1.p2 = pnt2
                line2 = None
            ...
            elif (mkDir.mkOUT) == dir & (
                mkDir.mkOUT
            ):  # default trim two sides of the arc
                line1.p1 = pnt1
                line1.p2 = pnt2
                line2 = None
    ...
    def draw(self, qp):
        ...
        qp.drawArc(
            int(self._qcen.x() - self._qrad),
            int(self._qcen.y() - self._qrad),
            int(self._qrad * 2),
            int(self._qrad * 2),
            int(self._sang * 16),
            int((self._eang - self._sang) * 16),
        )
