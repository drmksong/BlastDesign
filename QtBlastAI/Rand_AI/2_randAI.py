...
def lin_reg(x,y):
  ...
  lin_fit = LinearRegression()
  lin_fit.fit(logx,logy)

  n = lin_fit.coef_[0,0]
  logK = lin_fit.intercept_[0]

  sqr = np.sqrt(logK*logK+n*n)
  D = (n*logx-logy+logK)/sqr # distance to the regression line of every point
...
# creat 100 sequences with train window
...
#create sequence based on K, n, s of size num
def gen_seq(key_in, num):
  ...
  d = np.random.normal(0,s,num)

  X = np.random.lognormal(5,0.7,num) + 5
  logX = np.log(X)
  logY = (n*logX+logK) - d*sqr  
  Y = np.exp(logY)
  # Y = np.log(K*pow(X,n))
  ...
  print(f' original {s:3.3}, generated {s_check:3.3}')

  seq = np.stack([X,Y],axis=1)
  print(seq.size)

  KK, nn, ss = lin_reg(X.reshape(-1,1),Y.reshape(-1,1))
...
def main():
  ...
  plt.text(-11.0, -1.0, f'R^2 = {1.0:3.3}', fontsize=12)
  plt.title(f'Phase 2 ')
  plt.ylabel('Predicted Settlement')
  plt.xlabel('True Settlement')
  plt.grid(True)
  plt.autoscale(axis='x', tight=True)
  plt.scatter(seq[:,0],seq[:,1],c='#0088ff',alpha=0.7)
  plt.yscale('log')
  plt.xscale('log')
  # plt.xlim(5,200)
